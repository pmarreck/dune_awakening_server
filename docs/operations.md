...
## Live-world commands (`dune-world admin`)

The map server starts with Funcom's server-command channel on: `dune-server` writes a generated `ServerCommandsAuthToken` (kept in `runtime/secrets/server_commands_token`, 0600) and `server.NotificationSystem.Enabled=true` into its private `Engine.ini`. `dune-admin` wraps a command in Funcom's Version 2 envelope and has the game RabbitMQ node publish it (exchange `heartbeats`, routing key `notifications`, user `fls`, app `fls_backend`); the envelope travels through a 0600 file, never a command line. Players are named by character; the tool resolves their Funcom id.

```bash
dune-world admin players                    # ● online / ○ offline
dune-world admin say "Restart in 5 minutes" --duration 30
dune-world admin kick <player>              # or --all
dune-world admin teleport <player> X Y Z [YAW]
dune-world admin water <player> 5000
dune-world admin xp <player> 1000 [Combat|Crafting|Gathering|Exploration|Sabotage]
dune-world admin exec t.MaxFPS 5            # world-level engine command or variable (ServerExec)
dune-world admin worm <player>              # experimental: SandwormTargetPlayer in the player's context
dune-world admin raw <ServerCommand> [--player P] [Key=value[:int|:float]...]
```
